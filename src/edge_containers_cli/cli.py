...
context_ops = typer.Typer()
cli.add_typer(
    context_ops,
    name="context",
    help="Commands to store, show, switch the current context (defaults)",
)
...
def store(
    ...
    context_name: str = typer.Argument(
        ...,
        help="Name to store the context under",
        autocompletion=running_svc,
    ),
):
    ...
    args.store_config(context_name, params)
...
def show(ctx: typer.Context):
    ...
    params = ctx.parent.parent.params  # type: ignore
    for var in ENV:
        typer.echo(f"{var.value}: {params[var.name]}")
...
def load(
    context_name: str = typer.Argument(
        ...,
        help="The context to switch to",
        autocompletion=all_contexts,
    ),
):
    """Load a stored context"""
    args = ConfigController(globals.CONFIG_ROOT / globals.ENV_CONFIG)
    args.load_config(context_name)
